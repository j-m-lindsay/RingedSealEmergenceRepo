...
# Description: code to fit Hidden Markov Models (HMMs) to haul-out data from adult ringed seals, estimate emergence dates for individual 
...
m_daylength_air2m <- fitHMM(data = dat, nbStates = 2, dist = dist, Par0 = Par0_DL_air$Par,
                            estAngleMean = list(peak_ho_hr=TRUE), stateNames = stateNames, beta0=Par0_DL_air$beta, delta0=Par0_DL_air$delta, formula=formula_DL_air, knownStates = knownStates,fixPar=list(beta=fixedbeta2))
...
m_daylength_temp <- fitHMM(data = dat, nbStates = 2, dist = dist, Par0 = Par0_DL_rollT$Par,
                           estAngleMean = list(peak_ho_hr=TRUE), stateNames = stateNames, beta0=Par0_DL_rollT$beta, delta0=Par0_DL_rollT$delta, formula=formula_DL_rollT, knownStates = knownStates,fixPar=list(beta=fixedbeta2))
...
estCI_air[names(estCI_air)] <- list(list())
for(i in 1:nrow(airSeq)){ #at each air temperature value, estimate emergence probability using the best HMM for adults
  out <- CIreal(best_mod, covs=data.frame(air2m=airSeq[i,],daylength=18))$gamma #we have specified a daylength of 18 hours
  estCI_air$est[[i]] <- out$est
  estCI_air$lower[[i]] <- out$lower
  estCI_air$upper[[i]] <- out$upper
}
par(mfrow=c(1,1))
...
fig4c <- air_emerg %>%
  ...
  geom_line(aes(x=air2m,y=est),lwd=1)+
...
#now for daylength
DLSeq <- (data.frame(daylength=seq(min(dat$daylength),max(dat$daylength),length=101))) #make a sequence of daylengths
estCI_DL <- vector('list',3)
names(estCI_DL) <- c("est","lower","upper")
estCI_DL[names(estCI_DL)] <- list(list())
for(i in 1:nrow(DLSeq)){ #at each daylength value, estimate emergence probability using the best HMM for adults
  out <- CIreal(m_daylength_air2m, covs=data.frame(daylength=DLSeq[i,],air2m=0))$gamma #we have specified an air temperature of 0 degrees C
  estCI_DL$est[[i]] <- out$est
  estCI_DL$lower[[i]] <- out$lower
  estCI_DL$upper[[i]] <- out$upper
}
par(mfrow=c(1,1))
...
#emergence date & temperature
mean(emergence$emergtemp) #on average, what was the air temp when seals emerged
sd(emergence$emergtemp)
...
cor.test(emergence$emerglat,emergence$emergDL,method="spearman") #check correlation between latitude & daylength at time of emergence
DLvlat <-  lm(emergence$emergDL~emergence$emerglat) #relationship between latitude & daylength at time of emergence
DLvlat$coefficients[1]+DLvlat$coefficients[2]*66.5 #daylength at time of emergence for seals in Kotzebue Sound (~latitude 66.5)
DLvlat$coefficients[1]+DLvlat$coefficients[2]*70.5 #daylength at time of emergence for seals in Prudhoe Bay (~latitude 70.5)

#trends in emergence date by latitude and year
cor.test(emergence$emerglat,emergence$year,method="spearman") #check correlation between latitude & year
latmod <- lm(emerg~emerglat,data=emergence) #relationship between emergence date & latitude
yearmod <- lm(emerg~year,data=emergence) #relationship between emergence date & year
AICtab_lm <- (AIC(latmod,yearmod)) %>%
  mutate(npar=c(2,2))%>%
  arrange(AIC) %>%
  mutate(AICc=AIC+(2*npar^2 + 2*npar)/(nrow(emergence)-npar-1),dAICc=AICc-min(AICc))%>%
  select(dAICc) 
#latmod most supported per AICc
summary(latmod) #view latmod results
cor.test(resid(latmod),emergence$year,method="spearman") #check for pattern between latmod residuals and year

newdata = data.frame(emerglat=70.489722) #latitude of the Kelly et al. (2006) study area
kelly_pred <- predict.lm(latmod,newdata, interval="confidence") #predict emergence date
...
fig5 <- emergence %>%
  ...
  ylab("Emergence DOY")+
  xlab("Latitude")+
  theme(text=element_text(size=16), panel.background = element_blank(),  panel.grid.major = element_blank(),
                   panel.grid.minor = element_blank(),panel.border=element_blank(),axis.line = element_line(colour = "black"),
                   axis.text.y = element_text(angle=360, vjust=0.5, hjust=1), plot.margin = unit(c(20,20,20,20), "pt"))+
  labs(color='Year')+
...
ggsave(here::here("figures","figS2.png"),plot=FigS2,width=12,height=length(plots)*3,limitsize=FALSE )
